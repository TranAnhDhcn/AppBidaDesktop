...
namespace WindowsFormsAppBida.DAO
{
    public class BillDAO
    {
        ...
        public DataTable GetTotalRevenueDay(int date)
        {
            DataTable dt = DataProvider.Instance.ExecuteQuery("SELECT COUNT(*) as BillCountToday, SUM(totalPrice) as TotalPriceToday\r\nFROM Bill\r\nWHERE DAY(DateCheckIn) = " + date);
            return dt;
        }
        ...
        {
            DataTable dt = DataProvider.Instance.ExecuteQuery("SELECT COUNT(*) AS TotalBills, SUM(totalPrice) AS TotalPrice FROM Bill WHERE DateCheckIn >= '"+ dateIn +"' AND DateCheckIn <= '"+ dateOut +"'");
            return dt;
        }
    }
}
