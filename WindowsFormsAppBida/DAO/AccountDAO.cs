...
namespace WindowsFormsAppBida.DAO
{
    public class AccountDAO
    {
        ...
        public bool LoginCard(string idCard)
        {
            string query = "EXEC USP_LoginCard   @idCard ";

            DataTable result = DataProvider.Instance.ExecuteQuery(query, new object[] { idCard});
        ...
        public void updateStatusAccIdCard(string idCard, int status)
        {
            string query = "UPDATE Account SET status = " + status + " WHERE idCard = '" + idCard + "'  ";
            DataTable result = DataProvider.Instance.ExecuteQuery(query);
        }
        ...
        {
            DataTable data = DataProvider.Instance.ExecuteQuery("Select * from account where active = 1 AND idCard = '" + idCard + "'");
        ...
        public bool InsertAdmin(string email, string name, int sex, string PassWord, int phone, int type, byte[] image)
        {
            byte[] temp = Encoding.UTF8.GetBytes(PassWord);
            byte[] hasData = new SHA256CryptoServiceProvider().ComputeHash(temp);
            ...
            string query = "USP_InsertAdminAcc @email , @name , @sex , @PassWord , @phone , @type , @image ";

            int result = DataProvider.Instance.ExecuteNonQuery(query, new object[] { email, name, sex, hasPass, phone, type, image });
